...
private:
  ...
  map<int, string> codes; // codeword for each char

public:
  void Encode( const char *inputFilename, const char *outputFilename )
  {
    map<int, int> freqs; // frequency for each char from input text
    int i;
    ...
    for( fi=freqs.begin(),i=0; fi!=freqs.end(); ++fi,++i )
    {
      ...
      ptable[i].p = double((*fi).second) / ftot;
    }
    ...
    for( i=0; i<tsize; i++ )
    {
      ...
      fprintf(outputFile, "%d: %s"NL, ptable[i].ch, codes[ptable[i].ch].c_str() );
    }
